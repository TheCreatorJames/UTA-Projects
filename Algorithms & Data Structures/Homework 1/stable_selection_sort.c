...
int main()
{
  // Create Buffers to Store the input
  char *text, *text2;

  // this is used to take input.
  text = malloc(LINE_SIZE);

  // this is used to parse the records
  text2 = malloc(LINE_SIZE);

  // asks for the number of records, and parses it into an integer. 
  ...
  for(iter = 0; iter < count; iter++)
  {
    ...
    while(text[offset] != 0)
    {
      ...
      //Parses the integer if there is a comma.
      // I swear the prompt was changed on us at some point, so I have support for both file formats.
